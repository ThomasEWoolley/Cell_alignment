...
for j=1:length(Starts)
    ...
    for i=1:length(Starts(j):Ends(j))
        x=Z(Starts(j):Ends(j),6);y=Z(Starts(j):Ends(j),7);
        theta=deg2rad(Z(Starts(j):Ends(j),17));
        y=max(y)-y;
        xy_vec=[x,y];
        point=xy_vec(i,:);
        xy_vec(i,:)=[nan nan];
...
g = fittype( ['exp(1/(',num2str(K(1)),'+k).*cos((deg2rad(x*2)-m)))./(180*besseli(0,1/(',num2str(K(1)),'+k)))'], 'coeff',{'m','k'} );
...
for kk=1
    ...
    m=ft{kk}.m;
    k=ft{kk}.k;
    gg=exp(1/(K(1)+k).*cos((deg2rad(tt*2)-m)))./(180*besseli(0,1/(K(1)+k)));
    ...
    hold on
    plot(linspace(-90,90),exp(1/(K(1)+k).*cos((deg2rad(linspace(-90,90)*2)-m)))./(180*besseli(0,1/(K(1)+k)))*prob,'r')
    axis(ax)
    legend('off')
